...
//        String query = "select Nome from usuario join contas on usuario.idusuario = contas.usuario_idusuario where Nome = 'Alan and nome > 1'";
        String query = "SELECT Numero, Cep, contas.descricao FROM Usuario JOIN Contas ON Usuario.idUsuario = Contas.Usuario_idUsuario WHERE Numero > 2 and Numero < 5 ORDER BY Nome ASC";
